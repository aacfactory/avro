...
func (c *SchemaCache) addProcessing(name string, schema Schema) {
	c.processingCache.Store(name, schema)
}

func (c *SchemaCache) getProcessing(name string) Schema {
	if v, ok := c.cache.Load(name); ok {
		return v.(Schema)
	}
	if v, ok := c.processingCache.Load(name); ok {
		return v.(Schema)
	}
	return nil
}
